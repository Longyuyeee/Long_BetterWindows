...
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
    throw "QuickLaunch test project was not found: $projectPath"
}
...
$temporaryBase = [System.IO.Path]::GetFullPath((Join-Path (
    [System.IO.Path]::GetTempPath()) "LongAssistant-QuickLaunch-TestResults"))
...
$trxPath = Join-Path $runRoot "quick-launch.trx"

try {
    $testOutput = @(& $DotnetPath test $projectPath `
        -c Release `
        --no-build `
        --filter "FullyQualifiedName~QuickLaunchIsolationTests" `
        --logger "trx;LogFileName=quick-launch.trx" `
        --results-directory $runRoot 2>&1)
    $testExitCode = $LASTEXITCODE
    if (-not (Test-Path -LiteralPath $trxPath -PathType Leaf)) {
        throw "QuickLaunch isolation tests produced no TRX report. Exit code: $testExitCode`n$($testOutput -join "`n")"
    }
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    ...
    $OutputPath = "artifacts/quality/quick-launch-isolation-$stamp/report.json"
}
...
Write-Host "QuickLaunch isolation matrix: $($evidence.passed_case_count)/$ExpectedCaseCount cases passed"
