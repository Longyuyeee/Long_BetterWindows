...
<#
.SYNOPSIS
  Measure idle host memory with the 20 distinct built-in Long plugins.
#>
param(
...
if (@(Get-ChildItem -LiteralPath $pluginRoot -Directory).Count -ne 20 `
    -or $manifestFiles.Count -ne 20 -or $uniqueIds.Count -ne 20) {
    throw "Memory probe requires 20 distinct built-in plugins: manifests=$($manifestFiles.Count), unique=$($uniqueIds.Count)"
}
...
$passed = @($results | Where-Object { $_.plugins -ne 20 -or $_.commands -lt 37 }).Count -eq 0 `
...
Write-Output "20-plugin memory probe: median=$median MB, max=$maximum MB, passed=$passed"
Write-Output "Report: $reportPath"
if (-not $passed) { throw '20-plugin memory probe did not meet the release threshold.' }
