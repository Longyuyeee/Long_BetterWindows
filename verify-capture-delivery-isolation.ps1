...
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
    throw "Capture delivery test project was not found: $projectPath"
}
...
$temporaryBase = [System.IO.Path]::GetFullPath((Join-Path (
    [System.IO.Path]::GetTempPath()) "LongAssistant-CaptureDelivery-TestResults"))
...
$trxPath = Join-Path $runRoot "capture-delivery.trx"

try {
    $testOutput = @(& $DotnetPath test $projectPath `
        -c Release `
        --no-build `
        --filter "FullyQualifiedName~AsyncDeliveryBoundaryTests|FullyQualifiedName~ColorPickerIsolationTests|FullyQualifiedName~ScreenshotIsolationTests" `
        --logger "trx;LogFileName=capture-delivery.trx" `
        --results-directory $runRoot 2>&1)
    $testExitCode = $LASTEXITCODE
    if (-not (Test-Path -LiteralPath $trxPath -PathType Leaf)) {
        throw "Capture delivery tests produced no TRX report. Exit code: $testExitCode`n$($testOutput -join "`n")"
    }
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    ...
    $OutputPath =
        "artifacts/quality/capture-delivery-isolation-$stamp/report.json"
}
...
Write-Host "Capture delivery isolation: $($evidence.passed_case_count)/$ExpectedCaseCount cases passed"
