...
当前状态：部分完成。已完成 `host.getInfo`、宿主绑定身份上下文、基础 Widget 方法、实例状态大小限制、Widget 事件信封、注入脚本 `CustomEvent` 派发、1 MiB Bridge 消息入口限制、受控 `https://plugin-*.longplugin.local/` 虚拟来源、WebView2 子资源来源阻断、HTML 文档响应头级 CSP、基础 CSP meta、Widget 生命周期协调器基础、Widget 实例状态持久化基础、Web SDK 类型和 Mock Host；尚未完成最终 Widget Surface 的真实尺寸/可见性接线、参考 Widget `.lpak` 和端到端验收。
