...
- Widget 实例、尺寸、可见性、暂停/恢复等生命周期；（PR-B 已完成生命周期协调器基础，仍未接最终 Widget Surface 的真实尺寸/可见性来源）
- `host.getInfo` 和 Widget Bridge；（PR-B 已完成 `host.getInfo`、基础 Widget 方法、受信上下文绑定、标准事件信封、注入脚本事件派发、Bridge 消息大小边界、虚拟 HTTPS 来源、响应头级 CSP 基础、基础 CSP meta 和 ready 回执；持久化仍未完成）
...
当前状态：部分完成。已完成 `host.getInfo`、宿主绑定身份上下文、基础 Widget 方法、实例状态大小限制、Widget 事件信封、注入脚本 `CustomEvent` 派发、1 MiB Bridge 消息入口限制、受控 `https://plugin-*.longplugin.local/` 虚拟来源、WebView2 子资源来源阻断、HTML 文档响应头级 CSP、基础 CSP meta、Widget 生命周期协调器基础、Web SDK 类型和 Mock Host；尚未完成最终 Widget Surface 的真实尺寸/可见性接线、实例状态持久化、参考 Widget `.lpak` 和端到端验收。
