...
本规范用于统一 Long助手 后续的产品交互、视觉设计、插件 UI、动效、主题、可访问性和工程实现方式。

Long助手 的目标不是成为一组互不相关的小工具，而是成为一个运行在 Windows 之上的效率增强中枢：用户可以随时唤起、输入或选择内容、获得匹配能力、完成操作，并立即返回原工作状态。
...
Long助手 可以参考 uTools 的统一入口、功能指令、输入智能匹配、插件生态和低步骤交互逻辑。
...
- uTools 官网的营销页面视觉。
- uTools 软件界面的具体布局和素材。
- 与 Long助手 Windows 深度集成目标无关的产品结构。

视觉品牌必须属于 Long助手；交互参考用于解决效率问题，而不是进行外观复刻。
