...
当前源码候选版本为 `v1.11.0-rc.5`，包含“Long助手”品牌、全局图标、安全更新链路、统一工作区、完整插件平台、LPWP 1.0 Long助手侧实现及插件逐项加固。`rc.2`、本地 `rc.3` 与绑定提交 `056d43a` 的 `rc.4` 均为不可覆盖的历史候选；当前代码必须从干净 `master` 构建新的 rc.5 unsigned 候选后再进行人工验收。RC5 并非最终稳定版。Windows 安装包暂未使用 Authenticode 商业证书；首次运行可能出现 SmartScreen 提示，请核对 SHA-256。
