...
[最新开发审计](docs/当前开发审计_2026-09-03.md) ·
...
当前源码版本为 `v1.11.0-rc.17`。该候选保留为不可覆盖的历史候选，不再继续人工回执，也不能直接改名为稳定版。截至 2026-09-03，纯自动验收迁移 P0-A001 至 A010 已完成，统一自动收口入口在远端 CI 对干净提交实现 94 / 94 门禁通过；A011-A025 完成后再从干净提交生成新候选。Windows 安装包暂未使用 Authenticode 商业证书；首次运行可能出现 SmartScreen 提示，请核对 SHA-256。
