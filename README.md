...
[插件开发](docs/插件开发指南.md) ·
[开发状态](docs/当前开发状态.md) ·
[uTools 对标与自动验收总计划](docs/uTools对标差距与自动化验收开发总计划_2026-08-15.md) ·
[最新开发审计](docs/当前开发审计与后续计划_2026-08-23.md) ·
...
当前源码版本为 `v1.11.0-rc.17`。该候选已经完成视觉、候选资产、桌面、安装器、任务栏与托盘等机器门禁，但项目在 2026-08-15 决定取消人工签字型发布门禁并重建纯自动验收合同，因此 RC17 保留为不可覆盖的历史候选，不再继续人工回执，也不能直接改名为稳定版。下一阶段从 P0 自动验收迁移开始，完成后重新生成候选。Windows 安装包暂未使用 Authenticode 商业证书；首次运行可能出现 SmartScreen 提示，请核对 SHA-256。
