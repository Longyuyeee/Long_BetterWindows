...
当前源码版本为 `v1.11.0-rc.15`，用于稳定版前最后一次收尾演练。冻结的 `rc.2` 至 `rc.14` 均为不可覆盖的历史候选；RC15 必须从精确干净提交生成全套新资产，不得复用或覆盖旧目录。当前并非最终稳定版。Windows 安装包暂未使用 Authenticode 商业证书；首次运行可能出现 SmartScreen 提示，请核对 SHA-256。
