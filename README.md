...
当前源码版本为 `v1.11.0-rc.7` 开发态，包含“Long助手”品牌、全局图标、安全更新链路、统一工作区、完整插件平台、LPWP 1.0 Long助手侧实现及启动器连续性加固。冻结的 `rc.2` 至 `rc.7` 均为不可覆盖的历史候选；RC7 冻结后已有 UI 统一修复进入主线，下一次候选构建必须先提升版本身份，不得覆盖旧目录。当前并非最终稳定版。Windows 安装包暂未使用 Authenticode 商业证书；首次运行可能出现 SmartScreen 提示，请核对 SHA-256。
